...
(define (mupllist->racketlist mu-list)
  (cond [(aunit? mu-list) null]
        [(apair? mu-list) (cons (apair-e1 mu-list) (mupllist->racketlist (apair-e2 mu-list)))]))
  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([e1 (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? e1)
               (eval-under-env (fun-body (closure-fun e1)) (if (fun-nameopt (closure-fun e1))
                                                               (cons (cons (fun-formal (closure-fun e1)) e2) (cons (cons (fun-nameopt (closure-fun e1)) e1) (closure-env e1)))
                                                               (cons (cons (fun-formal (closure-fun e1)) e2) (closure-env e1))))
               (error "Expression not a closure")))]
        [(apair? e)
